...
if ( version_compare( phpversion(), '5.3.0', '<' ) ) {
	...
	function bcn_phpold() {
		 printf( '<div class="notice notice-error"><p>' . __( 'Your PHP version is too old, please upgrade to a newer version. Your version is %1$s, Breadcrumb NavXT requires %2$s', 'wokiee-core' ) . '</p></div>', phpversion(), '5.3.0' );
	}
...
class bcn_rest_controller {
	 ...
	 */
	protected function register_rest_route( $namespace, $route, $args = array(), $override = false, $endpoint = null ) {
	...
	public function register_routes() {
		$this->register_rest_route(
			$this->unique_prefix . '/v' . $this::version,
			'/post/(?P<id>[\d]+)',
			array(
				'args'                => array(
					'id' => array(
						'description'       => __( 'The ID of the post (any type) to retrieve the breadcrumb trail for.', 'wokiee-core' ),
						'type'              => 'integer',
						'required'          => true,
		...
		$this->register_rest_route(
			$this->unique_prefix . '/v' . $this::version,
			'/term/(?P<taxonomy>[\w-]+)/(?P<id>[\d]+)',
			array(
				'args'     => array(
					'id'       => array(
						'description'       => __( 'The ID of the term to retrieve the breadcrumb trail for.', 'wokiee-core' ),
						'type'              => 'integer',
						'required'          => true,
						'validate_callback' => array( $this, 'validate_id' ),
					),
					'taxonomy' => array(
						'description'       => __( 'The taxonomy of the term to retrieve the breadcrumb trail for.', 'wokiee-core' ),
						'type'              => 'string',
						'required'          => true,
		...
		$this->register_rest_route(
			...
			array(
				'args'     => array(
					'id' => array(
						'description'       => __( 'The ID of the author to retrieve the breadcrumb trail for.', 'wokiee-core' ),
						'type'              => 'integer',
						'required'          => true,
						'validate_callback' => array( $this, 'validate_id' ),
					),
